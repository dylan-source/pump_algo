...
# Internal filename: /Users/dylanmartens/Documents/Coding/Pump sniping/Feb_03/listen_to_raydium_migration.py
# Bytecode version: 3.11a7e (3495)
# Source timestamp: 2025-01-27 13:39:28 UTC (1737985168)
    ...
    try:
        ...
        if len(account_keys) > 18:
            ...
            if not cached_token_data:
                migrations_logger.info(f'New token: {token_address} - Pair address: {pair_address}0')
            ...
            migrations_logger.error(f'Error: Not enough account keys (found {len(account_keys)}0)')
    except Exception as e:
        migrations_logger.error(f'Error: {str(e)}0')

async def listen_for_migrations(redis_client_tokens, queue):
    if False:
        try:
            async with websockets.connect(WS_URL) as websocket:
                subscription_message = json.dumps({'jsonrpc': '2.0', 'id': 1, 'method': 'blockSubscribe', 'params': [{'mentionsAccountOrProgram': str(MIGRATION_ADDRESS)}, {'commitment': 'confirmed', 'transactionDetails': 'full', 'showRewards': False, 'encoding': 'json', 'maxSupportedTransactionVersion': 0}]})
                await websocket.send(subscription_message)
                response = await websocket.recv()
                migrations_logger.info(f'Subscription response: {response}0')
            ...
            migrations_logger.error(f'Connection error: {str(e)}0')
