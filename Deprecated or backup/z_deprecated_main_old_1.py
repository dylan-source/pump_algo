...
from config import logger, RPC_URL, SOL_MINT, TRADE_AMOUNT_SOL, SOL_DECIMALS, WALLET_ADDRESS, PRIVATE_KEY, BUY_SLIPPAGE_BPS, SOL_AMOUNT, USDC_MINT
...
from z_deprecated_listener_wss_utils import monitor_transactions
...
async def main():
    ...
    producer_task = asyncio.create_task(monitor_transactions(queue, redis_client, httpx_client))
    consumer_task = asyncio.create_task(consume_queue(queue))
